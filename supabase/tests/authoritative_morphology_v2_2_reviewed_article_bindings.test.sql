-- D10 pending pgTAP: validate all 73 Category-A article bindings as one exact set.
begin;
...
create temporary table expected_d10_reviewed_bindings (
  rollout_order integer primary key,
  lexeme_id uuid unique not null,
  normalized_lemma text not null,
  pos text not null,
  article_id bigint not null,
  rejected_article_ids bigint[] not null,
  semantic_evidence text not null
) on commit drop;

insert into expected_d10_reviewed_bindings values
...
select is((select count(*) from expected_d10_reviewed_bindings), 73::bigint,
  'fixture contains exactly 73 reviewed Category-A decisions');

select is((
  select count(*)
  from expected_d10_reviewed_bindings expected
  join private.authoritative_morphology_article_bindings_v2 binding
    on binding.lexeme_id = expected.lexeme_id
   and binding.dictionary_code = 'bm'
   and binding.article_id = expected.article_id
   and binding.normalized_lemma = expected.normalized_lemma
   and binding.pos = expected.pos
   and binding.is_active
), 73::bigint, 'all reviewed bindings exist with exact identity dimensions');

select is((
  select count(*)
  from private.authoritative_morphology_article_bindings_v2 binding
  where binding.lexeme_id in (select lexeme_id from expected_d10_reviewed_bindings)
    and binding.dictionary_code = 'bm'
    and binding.is_active
), 73::bigint, 'each reviewed lexeme has exactly one active BM binding');

select is((
  select count(*)
  from expected_d10_reviewed_bindings expected
  join public.lexemes lexeme on lexeme.id = expected.lexeme_id
  where lexeme.lemma = expected.normalized_lemma
    and lexeme.pos = expected.pos
), 73::bigint, 'every fixture decision matches the learner lemma and POS');

select is((
  select count(*)
  from expected_d10_reviewed_bindings expected
  join private.authoritative_morphology_article_bindings_v2 binding
    on binding.lexeme_id = expected.lexeme_id and binding.article_id = expected.article_id
  where binding.provider_version = 'authoritative-article-binding/v1'
    and binding.evidence_ids[1] = 'ordbokene:bm:' || expected.article_id::text
    and binding.evidence_ids[2] = 'manual:lexeme-semantic-binding:' || expected.semantic_evidence
), 73::bigint, 'every binding retains exact source and semantic evidence identifiers');
...
select is((
  ...
  join private.authoritative_morphology_article_bindings_v2 binding
    on binding.lexeme_id = expected.lexeme_id and binding.article_id = expected.article_id
  where binding.evidence ->> 'reviewCategory' = 'A'
    and (binding.evidence ->> 'acceptedArticleId')::bigint = expected.article_id
    and binding.evidence -> 'rejectedHomographArticleIds' = to_jsonb(expected.rejected_article_ids)
), 73::bigint, 'every binding records accepted and rejected source articles');
...
select is((
  ...
  join private.authoritative_morphology_article_bindings_v2 binding
    on binding.lexeme_id = expected.lexeme_id and binding.article_id = expected.article_id
  where jsonb_typeof(binding.evidence -> 'learnerMeaning') = 'object'
    and nullif(binding.evidence #>> '{learnerMeaning,example}', '') is not null
), 73::bigint, 'every binding preserves a non-empty learner example');

select is((
  select count(*)
  from expected_d10_reviewed_bindings expected
  join private.authoritative_morphology_article_bindings_v2 binding
    on binding.lexeme_id = expected.lexeme_id and binding.is_active
  cross join lateral unnest(expected.rejected_article_ids) rejected(article_id)
  where binding.article_id = rejected.article_id
), 0::bigint, 'no rejected homograph article is active for a reviewed lexeme');

select is((
  select count(*)
  from (
    select * from public.get_authoritative_morphology_article_bindings_v2(
      array(select lexeme_id from expected_d10_reviewed_bindings where rollout_order between 1 and 25 order by rollout_order)
    )
    union all
    select * from public.get_authoritative_morphology_article_bindings_v2(
      array(select lexeme_id from expected_d10_reviewed_bindings where rollout_order between 26 and 50 order by rollout_order)
    )
    union all
    select * from public.get_authoritative_morphology_article_bindings_v2(
      array(select lexeme_id from expected_d10_reviewed_bindings where rollout_order between 51 and 73 order by rollout_order)
    )
  ) reader
), 73::bigint, 'bounded service reader returns all 73 bindings in three batches');
