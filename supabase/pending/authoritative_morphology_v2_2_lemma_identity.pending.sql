-- D10 / AUTHORITATIVE MORPHOLOGY V2.2 (PENDING)
-- Preserve distinct official co-headword paradigms when Ordbokene reuses a
-- paradigm ID inside one article. This file must be promoted with
-- `supabase migration new` before production application.

alter table private.authoritative_morphology_paradigms_v2
  drop constraint authoritative_morphology_para_snapshot_id_dictionary_code_a_key;

alter table private.authoritative_morphology_paradigms_v2
  add constraint authoritative_morphology_paradigms_v2_source_identity_key
...
comment on constraint authoritative_morphology_paradigms_v2_source_identity_key
  on private.authoritative_morphology_paradigms_v2 is
  'One source paradigm per snapshot, dictionary, article, official lemma, POS, and paradigm ID.';

comment on function public.publish_authoritative_morphology_snapshot_v2(uuid, jsonb, jsonb, jsonb) is
  'Service-role-only atomic D10 publisher; permits source-verified co-headwords while rejecting missing exact lemmas, partial source results, Nynorsk, divergent projections, and invented forms.';
