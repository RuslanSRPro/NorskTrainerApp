...
Deno.test(
  "binding eligibility V1 exposes exact producer and version",
  () => {
    bindingEligibilityAssert(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_BINDING_ELIGIBILITY_V1 ===
        "canonical-runtime-token-pos-current-snapshot-bound-binding-eligibility-v1",
      "unexpected producer",
    );
...
Deno.test(
  "public derivation has exactly six source inputs",
  () => {
    bindingEligibilityAssert(
      deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundBindingEligibilityV1
        .length ===
        6,
      "public API must expose exactly six source inputs",
    );
  },
);

Deno.test(
  "layer independently reexecutes CLOSED filtering",
  () => {
    bindingEligibilityHas(
      /await\s+deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundOccurrenceFilteringV1\s*\(/,
      "CLOSED filtering derivation must be reexecuted",
    );

    bindingEligibilityHas(
      /closedOccurrenceFilteringPublicDerivationReexecuted:\s*true/,
      "governance must declare filtering reexecution",
    );

    bindingEligibilityHas(
      /suppliedOccurrenceFilteringAcceptedAsProof:\s*false/,
      "supplied filtering wrapper must not be accepted as proof",
    );
  },
);

Deno.test(
  "layer independently reexecutes CLOSED cardinality enforcement",
  () => {
    bindingEligibilityHas(
      /await\s+deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundCardinalityEnforcementV1\s*\(/,
      "CLOSED cardinality enforcement must be reexecuted",
    );

    bindingEligibilityHas(
      /closedCardinalityEnforcementPublicDerivationReexecuted:\s*true/,
      "governance must declare enforcement reexecution",
    );

    bindingEligibilityHas(
      /suppliedCardinalityEnforcementAcceptedAsProof:\s*false/,
      "supplied enforcement wrapper must not be accepted as proof",
    );
  },
);

Deno.test(
  "exact filtering-domain join uses enforcement sourceFilteringDomainId",
  () => {
    bindingEligibilityHas(
      /enforcementDomain\.sourceFilteringDomainId\s*===\s*filteringDomain\.filteringDomainId/,
      "exact filtering domain id join missing",
    );

    bindingEligibilityHas(
      /exactFilteringDomainJoinRequired:\s*true/,
      "governance must require exact filtering-domain join",
    );
  },
);

Deno.test(
  "exact snapshot and sentence lineage is checked across joined domains",
  () => {
    bindingEligibilityHas(
      /enforcementDomain\.snapshotIdentityId\s*===\s*filteringDomain\.snapshotIdentityId/,
      "snapshot identity join missing",
    );

    bindingEligibilityHas(
      /enforcementDomain\.snapshotSentenceOccurrenceIdentityId\s*===\s*filteringDomain\.snapshotSentenceOccurrenceIdentityId/,
      "snapshot sentence occurrence identity join missing",
    );

    bindingEligibilityHas(
      /enforcementDomain\.expectedSiteAuthorityId\s*===\s*filteringDomain\.expectedSiteAuthorityId/,
      "expected site authority join missing",
    );
  },
);

Deno.test(
  "binding eligibility requires proven satisfied true survivor",
  () => {
    bindingEligibilityHas(
      /occurrence\.filteringState\s*===\s*"survives"[\s\S]*occurrence\.survivesFiltering\s*===\s*true[\s\S]*occurrence\.disposition\s*===\s*"satisfied"[\s\S]*occurrence\.booleanTruthResolved\s*===\s*true[\s\S]*occurrence\.booleanTruth\s*===\s*true/,
      "eligible occurrence must require full proven true conjunction",
    );

    bindingEligibilityHas(
      /provenSatisfiedOnlyBindingEligible:\s*true/,
      "governance must encode proven-satisfied-only eligibility",
    );
  },
);

Deno.test(
  "unresolved unavailable and blocked survivors remain unresolved for binding",
  () => {
    bindingEligibilityHas(
      /occurrence\.disposition\s*===\s*"uncertain"[\s\S]*occurrence\.disposition\s*===\s*"unavailable"[\s\S]*occurrence\.disposition\s*===\s*"blocked"/,
      "unresolved survivor states must be explicit",
    );

    bindingEligibilityHas(
      /unresolvedSurvivorBlocksBindingReadiness:\s*true/,
      "unresolved survivor must block readiness",
    );
  },
);

Deno.test(
  "proven false filtered occurrence is not binding eligible",
  () => {
    bindingEligibilityHas(
      /occurrence\.filteringState\s*===\s*"filtered_out"[\s\S]*occurrence\.disposition\s*===\s*"rejected"[\s\S]*occurrence\.booleanTruthResolved\s*===\s*true[\s\S]*occurrence\.booleanTruth\s*===\s*false/,
      "filtered-out classification must require proven false rejection",
    );
  },
);

Deno.test(
  "domain readiness vocabulary is exactly ready unresolved_occurrences cardinality_violation",
  () => {
    bindingEligibilityHas(
      /export type CanonicalRuntimeTokenPosCurrentSnapshotBoundBindingReadinessStateV1\s*=\s*\|\s*"ready"\s*\|\s*"unresolved_occurrences"\s*\|\s*"cardinality_violation"/,
      "unexpected domain readiness vocabulary",
    );
  },
);

Deno.test(
  "cardinality satisfaction alone cannot imply binding readiness",
  () => {
    bindingEligibilityHas(
      /unresolvedSurvivingOccurrenceCount\s*>\s*0[\s\S]*readinessState\s*=\s*"unresolved_occurrences"/,
      "unresolved survivor check must remain after cardinality satisfaction",
    );

    bindingEligibilityHas(
      /cardinalitySatisfiedRequiredForBindingReadiness:\s*true/,
      "governance must require satisfied cardinality",
    );
  },
);

Deno.test(
  "cardinality violation is readiness evidence not blocking failure",
  () => {
    bindingEligibilityHas(
      /readinessState\s*=\s*"cardinality_violation"/,
      "cardinality violation readiness state missing",
    );

    bindingEligibilityHas(
      /cardinalityViolationIsBlockingFailure:\s*false/,
      "cardinality violation must not become structural blocking failure",
    );
  },
);

Deno.test(
  "ready domain requires every survivor to be eligible",
  () => {
    bindingEligibilityHas(
      /bindingReady\s*&&[\s\S]*eligibleOccurrenceCount\s*!==[\s\S]*filteringDomain\.survivingOccurrenceCount/,
      "ready domain must verify all survivors are eligible",
    );
  },
);

Deno.test(
  "singleton survivor is not treated as winner",
  () => {
    bindingEligibilityHas(
      /singletonSurvivorTreatedAsWinner:\s*false/,
      "singleton survivor must not become winner",
    );

    bindingEligibilityLacks(
      /survivingOccurrenceCount\s*===\s*1/,
      "singleton survivor shortcut is forbidden",
    );
...
Deno.test(
  "semantic ceiling stops before binding set materialization and final binding",
  () => {
    bindingEligibilityHas(
      /bindingOccurrenceSetMaterialized:\s*false/,
      "binding set must not be materialized here",
    );

    bindingEligibilityHas(
      /occurrenceBindingPerformed:\s*false/,
      "occurrence binding must remain false",
    );

    bindingEligibilityHas(
      /finalRuntimeOccurrenceBindingPerformed:\s*false/,
      "final Runtime binding must remain false",
    );

    bindingEligibilityHas(
      /whereEvaluationPerformed:\s*false/,
      "WHERE evaluation must remain false",
    );

    bindingEligibilityHas(
      /learnerErrorClassified:\s*false/,
...
Deno.test(
  "layer does not own graph mutation dependency generation or clause generation",
  () => {
    bindingEligibilityHas(
      /constraintPropagationInvoked:\s*false/,
      "constraint propagation must remain false",
    );

    bindingEligibilityHas(
      /canonicalDependencyEdgeGenerated:\s*false/,
      "dependency generation must remain false",
    );

    bindingEligibilityHas(
      /clauseNodeGenerated:\s*false/,
      "clause generation must remain false",
    );

    bindingEligibilityHas(
      /graphMutationPerformed:\s*false/,
      "graph mutation must remain false",
    );
  },
);

type BindingEligibilityFixtureOptions = {
  cardinality:
    | "one"
    | "one_or_more"
    | "zero_or_more";
...
async function runBindingEligibilityFixture(
  options: BindingEligibilityFixtureOptions,
) {
  const inputs = await makeV2Inputs({
...
function requireBindingEligibilityReady(
  result: Awaited<
    ReturnType<
  ...
  if (
    ...
    throw new Error(
      `Binding Eligibility fixture blocked: ${
        JSON.stringify(
          result,
        )
      }`,
...
Deno.test(
  "binding eligibility executable R1 one + one satisfied survivor is ready",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: SATISFIED_POS,
    });

    const result = requireBindingEligibilityReady(
      run.result,
    );
...
Deno.test(
  "binding eligibility executable R2 one + zero survivors is cardinality violation underflow",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "none",
...
Deno.test(
  "binding eligibility executable R3 one + three satisfied survivors is cardinality violation overflow",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
...
Deno.test(
  "binding eligibility executable R4 one_or_more + three satisfied survivors is ready",
  async () => {
    const run = await runBindingEligibilityFixture({
...
Deno.test(
  "binding eligibility executable R5 zero_or_more + zero survivors is ready with empty eligible set",
  async () => {
    const run = await runBindingEligibilityFixture({
...
Deno.test(
  "binding eligibility executable R6 zero_or_more + three satisfied survivors is ready",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "zero_or_more",
      occurrenceMode: "three",
...
Deno.test(
  "binding eligibility executable R7 cardinality satisfied + uncertain survivor remains unresolved",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    const occurrence = domain.occurrenceEligibility[0]!;
...
Deno.test(
  "binding eligibility executable R8 cardinality satisfied + unavailable survivor remains unresolved",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
...
Deno.test(
  "binding eligibility executable R9 cardinality satisfied + blocked survivor remains unresolved",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
...
Deno.test(
  "binding eligibility executable R10 proven false occurrence is filtered out and never eligible",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
...
Deno.test(
  "binding eligibility executable R11 exact snapshot token occurrence identity is preserved",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    const sourceOccurrence = filtering.authority
      .domainFiltering[0]!
      .occurrenceFiltering[0]!;

    const derivedOccurrence = result.authority
      .domainBindingEligibility[0]!
      .occurrenceEligibility[0]!;

    fixtureAssert(
      derivedOccurrence.snapshotTokenOccurrenceIdentityId ===
          sourceOccurrence.snapshotTokenOccurrenceIdentityId &&
        derivedOccurrence.snapshotIdentityId ===
          sourceOccurrence.snapshotIdentityId &&
        derivedOccurrence.snapshotSentenceOccurrenceIdentityId ===
          sourceOccurrence.snapshotSentenceOccurrenceIdentityId,
      JSON.stringify({
        sourceOccurrence,
        derivedOccurrence,
      }),
...
Deno.test(
  "binding eligibility executable R12 exact filtering domain lineage is preserved",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    const sourceDomain = filtering.authority
      .domainFiltering[0]!;
    ...
    fixtureAssert(
      derivedDomain.sourceFilteringDomainId ===
          sourceDomain.filteringDomainId &&
        result.authority.sourceFilteringAuthorityId ===
          filtering.authority.authorityId &&
        derivedDomain.snapshotIdentityId ===
          sourceDomain.snapshotIdentityId &&
        derivedDomain.snapshotSentenceOccurrenceIdentityId ===
...
Deno.test(
  "binding eligibility executable R13 singleton eligible survivor still does not become winner or binding",
  async () => {
    const run = await runBindingEligibilityFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      result.authority
          ...
          1 &&
        g.singletonSurvivorTreatedAsWinner ===
          false &&
...
Deno.test(
  "binding eligibility executable R14 cardinality violation remains READY authority evidence",
  async () => {
    for (
      const occurrenceMode of [
        "none",
        "three",
      ] as const
    ) {
      ...
      fixtureAssert(
        domain.readinessState ===
          ...
          result.authority.governance
              .cardinalityViolationIsBlockingFailure ===
            false,
        JSON.stringify(
          result,
        ),
...
Deno.test(
  "binding eligibility executable R15 derivation is deterministic and does not mutate source inputs",
  async () => {
    const inputs = await makeV2Inputs({
      occurrenceMode: "one",
