...
type BindingValueFixtureOptions = {
  cardinality: unknown;
...
const VALUE_UNAVAILABLE_POS = {
  labels: [],
  statuses: [] as GraphStatus_V2[],
};

const VALUE_BLOCKED_POS = {
  labels: ["verb"],
  statuses: ["blocked"] as GraphStatus_V2[],
  alternativeStatus: "blocked" as const,
};

const VALUE_REJECTED_POS = {
  labels: ["noun"],
  statuses: ["resolved"] as GraphStatus_V2[],
  alternativeStatus: "resolved" as const,
  resolvedIndices: [0],
...
async function runBindingValueFixture(
  options: BindingValueFixtureOptions,
) {
  const inputs = await makeV2Inputs({
    occurrenceMode: options.occurrenceMode,
...
function requireBindingValueReady(
  result: Awaited<
    ReturnType<
...
type FinalBindingFixtureOptions = {
  cardinality: unknown;
...
const FINAL_UNAVAILABLE_POS = {
  labels: [],
  statuses: [] as GraphStatus_V2[],
};

const FINAL_BLOCKED_POS = {
  labels: ["verb"],
  statuses: ["blocked"] as GraphStatus_V2[],
  alternativeStatus: "blocked" as const,
};

const FINAL_REJECTED_POS = {
  labels: ["noun"],
  statuses: ["resolved"] as GraphStatus_V2[],
  alternativeStatus: "resolved" as const,
  resolvedIndices: [0],
...
async function runFinalBindingFixture(
  options: FinalBindingFixtureOptions,
) {
  const inputs = await makeV2Inputs({
    occurrenceMode: options.occurrenceMode,
...
function requireFinalBindingReady(
  result: Awaited<
    ReturnType<
  ...
  if (
    ...
    throw new Error(
      `Final Runtime Binding fixture blocked: ${
        JSON.stringify(
          result,
        )
      }`,
...
type BoundConditionFixtureOptions = {
  cardinality: unknown;
...
const BOUND_CONDITION_MATCH_POS = {
  labels: ["verb"],
  statuses: ["resolved"] as GraphStatus_V2[],
  alternativeStatus: "resolved" as const,
  resolvedIndices: [0],
...
const BOUND_CONDITION_NON_MATCH_POS = {
  labels: ["noun"],
  statuses: ["resolved"] as GraphStatus_V2[],
  alternativeStatus: "resolved" as const,
  resolvedIndices: [0],
...
const BOUND_CONDITION_UNCERTAIN_POS = {
  labels: ["verb", "noun"],
  statuses: [
    "candidate",
    "candidate",
...
async function runBoundConditionFixture(
  options: BoundConditionFixtureOptions,
) {
  const inputs = await makeV2Inputs({
    occurrenceMode: options.occurrenceMode,
...
function requireBoundConditionReady(
  result: Awaited<
    ReturnType<
...
Deno.test(
  "bound condition evidence V1 exposes exact producer version and six-input API",
  () => {
    assertEquals(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_BOUND_CONDITION_EVIDENCE_V1,
      "canonical-runtime-token-pos-current-snapshot-bound-bound-condition-evidence-v1",
    );
...
Deno.test(
  "bound condition evidence V1 independently reexecutes CLOSED Final Binding and Comparison V2",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",

      occurrenceMode: "one",

      pos: BOUND_CONDITION_MATCH_POS,
    });

    const result = requireBoundConditionReady(
      run.result,
    );
    ...
    fixtureAssert(
      g.closedFinalRuntimeBindingPublicDerivationReexecuted ===
          true &&
        g.suppliedFinalRuntimeBindingAcceptedAsProof ===
          false &&
        g.closedCurrentSnapshotBoundComparisonPublicDerivationReexecuted ===
          true &&
        g.suppliedComparisonAcceptedAsProof ===
          false,
      JSON.stringify(
        g,
      ),
...
Deno.test(
  "bound condition evidence V1 consumes one-member final binding and projects exact comparison truth",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
    ...
    const domain = result.authority
      .domainBoundConditionEvidence[0]!;
...
Deno.test(
  "bound condition evidence V1 does not resurrect proven-false occurrence after CLOSED filtering",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
      ...
      pos: BOUND_CONDITION_NON_MATCH_POS,
    });
    ...
    const sourceComparison = comparison.authority
      .domainComparisons[0]!
      .occurrenceComparisons[0]!;

    fixtureAssert(
      sourceComparison.comparisonState ===
          "explicit_resolved_non_match" &&
        sourceComparison.booleanTruthResolved ===
          true &&
        sourceComparison.booleanTruth ===
          false,
      JSON.stringify(
        sourceComparison,
      ),
    ...
    fixtureAssert(
      domain.sourceBindingState ===
        ...
        domain.boundConditionCollection ===
          null &&
        result.authority.projectedConditionMemberCount ===
          0 &&
        result.authority.governance
            .runtimeConditionTruthResolved ===
          false &&
        result.authority.governance
            .bindingTruthResolved ===
          false &&
        result.authority.governance
...
Deno.test(
  "bound condition evidence V1 preserves 1-to-1 snapshot occurrence identity",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
    ...
    const boundMember = finalBinding.authority
      .domainFinalRuntimeBinding[0]!
      .finalRuntimeBindingValue!
      .members[0]!;

    const evidenceMember = result.authority
      .domainBoundConditionEvidence[0]!
      .boundConditionCollection!
      .members[0]!;

    fixtureAssert(
      evidenceMember.snapshotTokenOccurrenceIdentityId ===
          boundMember.snapshotTokenOccurrenceIdentityId &&
        evidenceMember.tokenNodeId ===
          boundMember.tokenNodeId &&
        evidenceMember.containmentEdgeId ===
          boundMember.containmentEdgeId &&
        evidenceMember.sentenceTokenIndex ===
          boundMember.sentenceTokenIndex,
      JSON.stringify({
        boundMember,
        evidenceMember,
      }),
...
Deno.test(
  "bound condition evidence V1 one_or_more preserves exact three-member order",
  async () => {
    const run = await runBoundConditionFixture({
    ...
    const sourceMembers = finalBinding.authority
      .domainFinalRuntimeBinding[0]!
      .finalRuntimeBindingValue!
      .members;

    const evidenceMembers = result.authority
      .domainBoundConditionEvidence[0]!
      .boundConditionCollection!
      .members;
...
Deno.test(
  "bound condition evidence V1 zero_or_more empty final binding is consumed as empty condition collection",
  async () => {
    const run = await runBoundConditionFixture({
    ...
    fixtureAssert(
      domain.boundConditionDomainState ===
        ...
        domain.boundConditionCollection !==
          null &&
        domain.boundConditionCollection.memberCount ===
          0 &&
        domain.boundConditionCollection.isEmptyCollection ===
          true &&
        result.authority.projectedConditionMemberCount ===
          0 &&
        result.authority.governance
            .emptyFinalBindingSynthesizesOccurrenceEvidence ===
          false,
      JSON.stringify(
        result.authority,
      ),
...
Deno.test(
  "bound condition evidence V1 unresolved final binding is not consumed",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
      ...
      pos: BOUND_CONDITION_UNCERTAIN_POS,
    });
...
Deno.test(
  "bound condition evidence V1 cardinality underflow is not consumed",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
...
Deno.test(
  "bound condition evidence V1 cardinality overflow is not consumed",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
...
Deno.test(
  "bound condition evidence V1 preserves exact site binding and snapshot lineage",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
    ...
    const source = finalBinding.authority
      .domainFinalRuntimeBinding[0]!;
    ...
    fixtureAssert(
      evidence.expectedSiteAuthorityId ===
          source.expectedSiteAuthorityId &&
        evidence.bindingDefinitionAuthorityId ===
          source.bindingDefinitionAuthorityId &&
        evidence.referencedBindingName ===
...
Deno.test(
  "bound condition evidence V1 consumes comparison evidence without recomputing truth",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
    ...
    const source = comparison.authority
      .domainComparisons[0]!
      .occurrenceComparisons[0]!;
    ...
    fixtureAssert(
      evidence.sourceComparisonId ===
          source.comparisonId &&
        evidence.comparisonState ===
          source.comparisonState &&
        evidence.booleanTruthResolved ===
          source.booleanTruthResolved &&
        evidence.booleanTruth ===
          source.booleanTruth &&
        result.authority.governance
            .comparisonEvidenceConsumedNotRecomputed ===
          true,
      JSON.stringify({
        source,
        evidence,
...
Deno.test(
  "bound condition evidence V1 never treats tokenNodeId as cross-layer occurrence identity",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
    ...
    fixtureAssert(
      g.exactSnapshotTokenOccurrenceIdentityJoinRequired ===
          true &&
        g.tokenNodeIdUsedAsCrossLayerOccurrenceIdentity ===
          false,
      JSON.stringify(
        g,
      ),
...
Deno.test(
  "bound condition evidence V1 owns binding consumption but not runtime execution or truth aggregation",
  async () => {
    const run = await runBoundConditionFixture({
      cardinality: "one",
...
Deno.test(
  "bound condition evidence V1 unsupported cardinality remains blocked upstream",
  async () => {
    const run = await runBoundConditionFixture({
...
Deno.test(
  "bound condition evidence V1 derivation is deterministic and source inputs remain immutable",
  async () => {
    const inputs = await makeV2Inputs({
      occurrenceMode: "three",
