...
Deno.test(
  "binding value projection V1 exposes exact producer version and six-input API",
  () => {
    assertEquals(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_BINDING_VALUE_PROJECTION_V1,
      "canonical-runtime-token-pos-current-snapshot-bound-binding-value-projection-v1",
    );
...
Deno.test(
  "binding value projection V1 reexecutes CLOSED occurrence set and accepts no supplied wrapper",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: VALUE_SATISFIED_POS,
    });

    const result = requireBindingValueReady(
      run.result,
    );
    ...
    fixtureAssert(
      g.closedBindingOccurrenceSetPublicDerivationReexecuted ===
          true &&
        g.suppliedBindingOccurrenceSetAcceptedAsProof ===
          false &&
        g.exactBindingOccurrenceSetAuthorityConsumed ===
          true,
      JSON.stringify(
        g,
      ),
...
Deno.test(
  "binding value projection V1 one projects one-member collection without scalar collapse",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
...
Deno.test(
  "binding value projection V1 one_or_more preserves three-member collection order and multiplicity",
  async () => {
    const run = await runBindingValueFixture({
    ...
    const source = setResult.authority
    ...
    for (
      ...
      fixtureAssert(
        value.members[i]!
              .sourceBindingOccurrenceSetMemberId ===
            source[i]!
...
Deno.test(
  "binding value projection V1 zero_or_more zero projects legitimate empty collection value",
  async () => {
    const run = await runBindingValueFixture({
    ...
    fixtureAssert(
      domain.projectionState ===
        ...
        domain.bindingValue.members.length ===
          0 &&
        result.authority.governance
            .zeroMemberMaterializedSetProjectsEmptyCollectionValue ===
          true,
      JSON.stringify(
        domain,
      ),
...
Deno.test(
  "binding value projection V1 uncertain occurrence set projects no value",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: VALUE_UNCERTAIN_POS,
    });
...
Deno.test(
  "binding value projection V1 unavailable occurrence set projects no value",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: VALUE_UNAVAILABLE_POS,
    });
...
Deno.test(
  "binding value projection V1 blocked evidence occurrence set projects no value",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: VALUE_BLOCKED_POS,
    });
...
Deno.test(
  "binding value projection V1 underflow projects no value",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "none",
...
Deno.test(
  "binding value projection V1 overflow projects no value even when source occurrences are eligible",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "three",
...
Deno.test(
  "binding value projection V1 proven false occurrence projects no value",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: VALUE_REJECTED_POS,
    });
...
Deno.test(
  "binding value projection V1 preserves exact set authority domain and binding definition lineage",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      result.authority
          ...
          setResult.authority.authorityId &&
        derived.sourceBindingOccurrenceSetDomainId ===
          source.bindingOccurrenceSetDomainId &&
        derived.bindingDefinitionAuthorityId ===
          source.bindingDefinitionAuthorityId &&
        derived.expectedSiteAuthorityId ===
          source.expectedSiteAuthorityId &&
        derived.manifestId ===
          source.manifestId &&
        derived.manifestCode ===
          source.manifestCode &&
        derived.referencedBindingName ===
...
Deno.test(
  "binding value projection V1 preserves exact snapshot member identity",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    const derived = value.members[0]!;

    fixtureAssert(
      derived.sourceBindingOccurrenceSetMemberId ===
          source.bindingOccurrenceSetMemberId &&
...
Deno.test(
  "binding value projection V1 does not infer winner from singleton collection",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
...
Deno.test(
  "binding value projection V1 semantic ceiling stops before binding execution truth and WHERE",
  async () => {
    const run = await runBindingValueFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      g.bindingValueProjectionPerformed ===
          true &&
        g.bindingValueProjected ===
          true &&
        g.occurrenceBindingPerformed ===
          false &&
        g.finalRuntimeOccurrenceBindingPerformed ===
...
Deno.test(
  "binding value projection V1 unsupported cardinality remains blocked upstream",
  async () => {
    const run = await runBindingValueFixture({
    ...
    fixtureAssert(
      run.result.authority ===
        ...
        run.result.blockingReasons.some(
          (reason) =>
            reason.includes(
              "current_snapshot_bound_binding_occurrence_set:",
            ),
        ),
      JSON.stringify(
        run.result,
...
Deno.test(
  "binding value projection V1 derivation is deterministic and source inputs remain immutable",
  async () => {
    const inputs = await makeV2Inputs({
      occurrenceMode: "three",
