...
// ---------------------------------------------------------------------------
// Golden-only extension:
// nested Runtime WHERE -> A4.6a1 -> A4.6a1a
//                     -> C2 -> D2a -> D2b1-v3 -> A4.6a1c1
//
// No production authority is defined here.
// Historical Golden files are not modified.
// ---------------------------------------------------------------------------

type A46a1SourceAuthorityE2E = ReturnType<
  typeof deriveCanonicalRuntimeManifestBindingWhereShapeAuthoritiesV1
>["authorities"][number];

function bindingResultE2E(
...
function nestedWhereShapeChainE2E() {
  const binding = bindingResultE2E({
    any: [
      {
        op: "eq",
  ...
  const a46a1 = deriveCanonicalRuntimeManifestBindingWhereShapeAuthoritiesV1(
    binding,
  );
  ...
  assert(
    a46a1.status ===
        "ready" &&
      a46a1.authorities.length ===
        1,
    `expected exact ready A4.6a1 source: ${JSON.stringify(a46a1)}`,
  );

  const sourceAuthority = a46a1.authorities[0]!;
  ...
  assert(
    a46a1a.status ===
        "ready" &&
      a46a1a.authorities.length ===
        1 &&
      a46a1a.blockingReasons.length ===
        0,
    `expected exact ready A4.6a1a extension: ${JSON.stringify(a46a1a)}`,
  );
...
function joinedC2E2E(
  sourceAuthority: A46a1SourceAuthorityE2E,
  options: {
    leafPath: string;
    leafSiteId: string;
  ...
  return makeC2({
    site: {
      leafRightOperandSiteAuthorityId: options.leafSiteId,
    ...
    expectedOverrides: {
      whereShapeAuthorityId: sourceAuthority.id,

      leafPath: options.leafPath,
      ...
      ownerBindingName: sourceAuthority.bindingName,

      sourceOperatorLabelRaw: "eq",

      leftReferenceExpression: options.leftReferenceExpression,

      rightOperandSnapshot: options.rightOperandSnapshot,
...
function exactTargetDomainForJoinE2E(
  targetLeafSiteId: string,
  sourceAuthority: A46a1SourceAuthorityE2E,
) {
  return makeDomain({
...
function targetLeafV3E2E(
  ...
  const c2 = joinedC2E2E(
    sourceAuthority,
    {
      leafPath: NESTED_TARGET_LEAF_PATH,

      leafSiteId: targetSite,
  ...
  const domain = exactTargetDomainForJoinE2E(
    targetSite,
    sourceAuthority,
  );
  ...
  assert(
    d2a.status ===
      ...
      d2a.evidence
          .applicabilityState ===
        "unique_site_snapshot_token_occurrence_match",
    `expected exact target D2a: ${JSON.stringify(d2a)}`,
  );

  const v3 = deriveCanonicalRuntimeTokenPosConditionTruthCompositionV3(
    d2a,
    CANONICAL_RUNTIME_TOKEN_POS_EQ_CONDITION_TRUTH_SEMANTIC_CAPABILITY_RESULT_V2,
  );
  ...
  assert(
    v3.status ===
      ...
      v3.evidence !==
        undefined,
    `expected ready D2b1-v3: ${JSON.stringify(v3)}`,
  );
...
function siblingLeafV3E2E(
  ...
  const c2 = joinedC2E2E(
    sourceAuthority,
    {
      leafPath: NESTED_SIBLING_LEAF_PATH,

      leafSiteId: siblingSite,

      leftReferenceExpression: "subject.type",
  ...
  assert(
    d2a.status ===
      ...
      d2a.evidence
          .applicabilityState ===
        "no_matching_site_domain",
    `expected sibling D2a non-applicability: ${JSON.stringify(d2a)}`,
  );
  ...
  assert(
    v3.status ===
      ...
      v3.evidence
          .truthDisposition ===
        "unresolved",
    `expected unresolved sibling V3: ${JSON.stringify(v3)}`,
  );
...
Deno.test(
  "A4.6a1c1-e2e.1 real nested target lineage reaches exact ready childTruth nodePath",
  () => {
    const shape = nestedWhereShapeChainE2E();

    const truth = targetLeafV3E2E(
      shape.sourceAuthority,
    );
    ...
    const expected = truth.c2.evidence!
      .expectedAuthority;
    ...
    assert(
      result.status ===
        ...
        result.childTruth.status ===
          "ready" &&
        result.childTruth.evidence
            .nodePath ===
          NESTED_TARGET_LEAF_PATH &&
        result.childTruth.evidence
            .truthDisposition ===
          "resolved_true" &&
        result.matchedLeafNode.path ===
          NESTED_TARGET_LEAF_PATH &&
        result.matchedShapeAuthority
            .sourceWhereShapeAuthorityId ===
...
Deno.test(
  "A4.6a1c1-e2e.2 sibling remains isolated and binds only its own unresolved structural path",
  () => {
    const shape = nestedWhereShapeChainE2E();

    const truth = siblingLeafV3E2E(
      shape.sourceAuthority,
    );
...
Deno.test(
  "A4.6a1c1-e2e.3 corrupted preserved expected leafPath blocks instead of rebinding",
  () => {
    const shape = nestedWhereShapeChainE2E();
...
Deno.test(
  "A4.6a1c1-e2e.4 corrupted preserved whereShapeAuthorityId blocks instead of path-only rebinding",
  () => {
    const shape = nestedWhereShapeChainE2E();
    ...
    const truth = targetLeafV3E2E(
      shape.sourceAuthority,
      {
        whereShapeAuthorityId: "fixture:corrupt:where-shape-authority",
      },
    );
...
Deno.test(
  "A4.6a1c1-e2e.5 real nested bridge adds no compound manifest or cardinality semantics",
  () => {
    const shape = nestedWhereShapeChainE2E();
    ...
    assert(
      g.compoundTruthComposed ===
        ...
        g.cardinalityEnforcementPerformed ===
          false &&
        g.syntheticUnaryNotLeafTruthCreated ===
          false &&
        truth.v3.evidence !==
          undefined &&
        truth.v3.evidence
      ...
      JSON.stringify({
        ...
        sourceTruthGovernance: truth.v3.evidence?.governance,
      }),
    );
  },
);
