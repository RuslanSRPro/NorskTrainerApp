...
import {
  ...
  deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundWhereContextCoherentRecursiveCompoundTruthV1,
} from "./canonical-runtime-token-pos-current-snapshot-bound-where-context-coherent-recursive-compound-truth-v1.ts";
...
Deno.test(
  "CURRENT A4.6a1f orchestrator public identity and exact six-input API are frozen",
  () => {
    assert(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_WHERE_CONTEXT_COHERENT_RECURSIVE_COMPOUND_TRUTH_V1 ===
        "canonical-runtime-token-pos-current-snapshot-bound-where-context-coherent-recursive-compound-truth-v1",
      "producer mismatch",
    );
    ...
    assert(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_WHERE_CONTEXT_COHERENT_RECURSIVE_COMPOUND_TRUTH_SPECIFICATION_ID_V1 ===
        "v1.46:A4.6a1f-current:current-snapshot-bound-where-context-coherent-recursive-compound-truth-v1",
    ...
    assert(
      deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundWhereContextCoherentRecursiveCompoundTruthV1
        .length ===
        6,
      "public producer must expose exactly six canonical inputs",
    );
  },
);

Deno.test(
  "CURRENT A4.6a1f orchestrator source freezes semantic ceiling and exact source-reference handoff",
  async () => {
    const source = await Deno.readTextFile(
      new URL(
        "./canonical-runtime-token-pos-current-snapshot-bound-where-context-coherent-recursive-compound-truth-v1.ts",
        import.meta.url,
      ),
    );
    ...
    for (const marker of required) {
      assert(
        ...
        "missing frozen architecture marker: " + marker,
      );
    }
    ...
    const forbidden = [
      ...
      ".every(",
      ".some(",
      "sentenceIndexUsedAsContextIdentity: true",
      "winnerSelected: true",
    ...
    for (const marker of forbidden) {
      assert(
        !source.includes(marker),
        "forbidden architecture marker present: " + marker,
      );
    }
  },
);
...
function realSingletonTrueManifestRow(): CanonicalDependencyRuntimeManifestAuthorityRowV1 {
  return {
    ...
    ir_spec: {
      source: {
        primary_candidate_code: "test.real_singleton_true.source",
      },
...
Deno.test(
  "CURRENT A4.6a1f real singleton TRUE composes exact CURRENT context-bound evidence",
  async () => {
    ...
    const dependencyResult = deriveCanonicalDependencyRuntimeAuthoritiesV1(
      manifestRows,
    );
    ...
    const nodeTypeAuthority: CanonicalRuntimeManifestGraphNodeTypeAuthorityV1 =
      {
    ...
    const c2 = compareCanonicalTokenPosNormalizedLabelEvidenceV2(
      actualResult,
    ...
    const executionInput = {
      executionInvocationId: "real-canonical-singleton-true-e2e",
    ...
    const realExpected = expectedResult.authorities[0];
    ...
    v2Assert(
      ...
      `real producer Candidate blocked: ${JSON.stringify(candidate)}`,
    );

    v2Assert(
      candidate.authority.evidence.length === 1,
      `real producer Candidate evidence count=${candidate.authority.evidence.length}`,
    );

    v2Assert(
      candidate.authority.evidence[0]
        .childTruthEvidence
        .truthDisposition ===
        "resolved_true",
      `real producer Candidate did not materialize resolved_true: ${
        JSON.stringify(candidate.authority.evidence[0])
      }`,
    );
    ...
    v2Assert(
      serializedCandidate.includes(
        '"recursiveWhereCompositionPerformed":false',
      ),
      "Candidate unexpectedly performed recursive WHERE composition",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly composed compound truth",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly resolved manifest condition truth",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly resolved runtime condition truth",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly resolved binding truth",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly performed WHERE evaluation",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly classified learner error",
    );
    ...
    v2Assert(
      ...
      "Candidate unexpectedly mutated graph",
    );

    const contextBoundCandidate =
    ...
    v2Assert(
      contextBoundCandidate.status === "ready",
      `CURRENT A4.6a1e adapter blocked: ${
        JSON.stringify(contextBoundCandidate)
      }`,
    );

    if (contextBoundCandidate.status !== "ready") {
      throw new Error("CURRENT A4.6a1e adapter unexpectedly blocked");
    }

    v2Assert(
      contextBoundCandidate.evidenceCount === 1 &&
        contextBoundCandidate.evidence.length === 1,
      `CURRENT A4.6a1e evidence count=${contextBoundCandidate.evidence.length}`,
    );

    const contextBoundEvidence = contextBoundCandidate.evidence[0];

    v2Assert(
      contextBoundEvidence !== undefined,
      "CURRENT A4.6a1e evidence missing",
    );

    const sourceLeaf = contextBoundCandidate.sourceResult.authority.evidence[0];

    v2Assert(
      sourceLeaf !== undefined,
      "source CURRENT leaf evidence missing",
    );

    v2Assert(
      sourceLeaf.authorityBoundChildTruth.status === "ready",
      "source CURRENT authority-bound child truth not ready",
    );
    ...
    if (
      ...
      throw new Error(
        "source CURRENT authority-bound child truth unexpectedly blocked",
      );
    }

    const sourceAuthorityBoundEvidence =
      sourceLeaf.authorityBoundChildTruth.evidence;

    v2Assert(
      contextBoundEvidence.authorityBoundChildTruthEvidence ===
        sourceAuthorityBoundEvidence,
      "A4.6a1e did not preserve exact authority-bound evidence object reference",
    );

    v2Assert(
      contextBoundEvidence.structuralAuthorityId ===
          sourceAuthorityBoundEvidence.structuralAuthorityId &&
        contextBoundEvidence.nodePath ===
          sourceAuthorityBoundEvidence.nodePath,
      "A4.6a1e structural authority/path mismatch",
    );

    v2Assert(
      contextBoundEvidence.snapshotIdentityId ===
          currentDomainResult.authority.snapshotIdentityId &&
        contextBoundEvidence.snapshotSentenceOccurrenceIdentityId ===
          currentDomainResult.authority
            .snapshotSentenceOccurrenceIdentityId,
      "A4.6a1e CURRENT context composite identity mismatch",
    );

    v2Assert(
      contextBoundEvidence.graphDocumentId ===
          currentDomainResult.authority.graphDocumentId &&
        contextBoundEvidence.sentenceNodeId ===
          currentDomainResult.authority.sentenceNodeId,
      "A4.6a1e CURRENT context consistency metadata mismatch",
    );

    v2Assert(
      contextBoundEvidence.provenance
        .authorityBoundChildTruthEvidenceObjectPreservedWithoutReconstruction ===
        true,
      "A4.6a1e exact wrapped evidence preservation provenance missing",
    );

    v2Assert(
      contextBoundEvidence.provenance
            .authorityContextPairProvenBySourceAdapter ===
          true &&
        contextBoundEvidence.provenance
            .contextInferredByInterface ===
          false &&
        contextBoundEvidence.provenance
            .callerSuppliedContextAcceptedAsProof ===
          false,
      "A4.6a1e authority/context proof boundary mismatch",
    );
    ...
    v2Assert(
      contextBoundEvidence.provenance
        ...
        contextBoundEvidence.provenance
          ...
          false,
      "A4.6a1e unexpectedly duplicated or recomputed truth",
    );

    v2Assert(
      !("sentenceIndex" in contextBoundEvidence),
      "sentenceIndex leaked into A4.6a1e context evidence",
    );

    v2Assert(
      !("snapshotTokenOccurrenceIdentityId" in contextBoundEvidence),
      "token occurrence identity leaked into sentence context evidence",
    );

    v2Assert(
      sourceLeaf.childTruthEvidence.truthDisposition ===
        "resolved_true",
      "source real singleton truth was not resolved_true",
    );

    const serializedContextBound = JSON.stringify(contextBoundCandidate);

    v2Assert(
      serializedContextBound.includes(
        '"recursiveWhereCompositionPerformed":false',
      ) &&
        serializedContextBound.includes(
          '"compoundTruthComposed":false',
      ...
      "CURRENT A4.6a1e semantic ceiling mismatch",
    );

    const compoundResult =
      await deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundWhereContextCoherentRecursiveCompoundTruthV1(
        surface,
        graph,
        expectedResult,
        currentDomainResult,
    ...
    v2Assert(
      compoundResult.status === "ready",
      `CURRENT A4.6a1f orchestrator blocked: ${JSON.stringify(compoundResult)}`,
    );

    if (compoundResult.status !== "ready") {
      throw new Error(
        "CURRENT A4.6a1f orchestrator unexpectedly blocked",
      );
    }

    v2Assert(
      compoundResult.compositionResult.sourceLeafEvidence ===
        compoundResult.sourceContextBoundResult.evidence,
      "CURRENT A4.6a1f did not preserve exact A4.6a1e evidence-array handoff",
    );

    v2Assert(
      compoundResult.compositionResult.sourceShapeResult ===
        compoundResult.sourceShapeResult,
      "CURRENT A4.6a1f did not preserve exact rederived A4.6a1a source-result handoff",
    );

    v2Assert(
      compoundResult.compositionResult.authorityResults.length === 1,
      `CURRENT A4.6a1f authority result count=${compoundResult.compositionResult.authorityResults.length}`,
    );

    const compoundAuthority =
      compoundResult.compositionResult.authorityResults[0];

    v2Assert(
      compoundAuthority !== undefined &&
        compoundAuthority.status === "ready",
      "CURRENT A4.6a1f compound authority result not ready",
    );
    ...
    if (
      ...
      throw new Error(
        "CURRENT A4.6a1f compound authority unexpectedly unavailable",
      );
    }

    v2Assert(
      compoundAuthority.rootCandidates.length === 1,
      `CURRENT A4.6a1f root candidate count=${compoundAuthority.rootCandidates.length}`,
    );

    const rootCandidate = compoundAuthority.rootCandidates[0];

    v2Assert(
      rootCandidate !== undefined,
      "CURRENT A4.6a1f root candidate missing",
    );

    v2Assert(
      compoundResult.sourceContextBoundResult.status === "ready" &&
        compoundResult.sourceContextBoundResult.evidence.length === 1,
      "CURRENT A4.6a1f did not preserve one exact CURRENT A4.6a1e leaf evidence",
    );

    const compoundSourceLeafEvidence =
      compoundResult.sourceContextBoundResult.evidence[0];

    v2Assert(
      compoundSourceLeafEvidence !== undefined,
      "CURRENT A4.6a1f source CURRENT leaf evidence missing",
    );

    v2Assert(
      rootCandidate.rootTruth === compoundSourceLeafEvidence,
      "CURRENT A4.6a1f leaf root did not preserve exact external CURRENT context-bound evidence object",
    );

    v2Assert(
      rootCandidate.rootTruth.authorityBoundChildTruthEvidence
        .childTruthEvidence.truthDisposition === "resolved_true",
      `CURRENT A4.6a1f real singleton root truth=${rootCandidate.rootTruth.authorityBoundChildTruthEvidence.childTruthEvidence.truthDisposition}`,
    );

    const compoundSourceLeaf =
      compoundResult.sourceContextBoundResult.sourceResult.authority
        .evidence[0];

    v2Assert(
      compoundSourceLeaf !== undefined &&
        compoundSourceLeaf.authorityBoundChildTruth.status === "ready",
      "CURRENT A4.6a1f source authority-bound leaf truth not ready",
    );
    ...
    if (
      ...
      throw new Error(
        "CURRENT A4.6a1f source authority-bound leaf truth unexpectedly unavailable",
      );
    }

    v2Assert(
      compoundSourceLeafEvidence.authorityBoundChildTruthEvidence ===
        compoundSourceLeaf.authorityBoundChildTruth.evidence,
      "CURRENT A4.6a1f path lost exact authority-bound evidence reference",
    );
  },
);
