...
Deno.test(
  "v1.47 cardinality enforcement V1.1 public API requires six exact source inputs",
  () => {
    assertEquals(
      deriveCanonicalRuntimeTokenPosCurrentSnapshotBoundCardinalityEnforcementV1
        .length,
...
Deno.test(
  "v1.47 cardinality enforcement V1.2 producer and version are exact",
  () => {
    assertEquals(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_CARDINALITY_ENFORCEMENT_V1,
      "canonical-runtime-token-pos-current-snapshot-bound-cardinality-enforcement-v1",
    );
...
Deno.test(
  "v1.47 cardinality enforcement V1.3 one with zero survivors is underflow",
  async () => {
    const result = await runCardinalityBehaviour(
      "one",
      "none",
    );
    ...
    const domain = result.authority.domainCardinalityEnforcement[0]!;

    fixtureAssert(
      domain.survivingOccurrenceCount === 0,
    ...
    fixtureAssert(
      domain.enforcementState === "underflow" &&
        domain.cardinalitySatisfied === false,
      JSON.stringify(domain),
    );

    fixtureAssert(
      result.authority.underflowDomainCount === 1 &&
        result.authority.satisfiedDomainCount === 0 &&
        result.authority.overflowDomainCount === 0,
...
Deno.test(
  "v1.47 cardinality enforcement V1.4 one with one survivor is satisfied but not winner",
  async () => {
    const result = await runCardinalityBehaviour(
      "one",
      "one",
    ...
    fixtureAssert(
      domain.survivingOccurrenceCount === 1 &&
        domain.enforcementState === "satisfied" &&
        domain.cardinalitySatisfied === true,
      JSON.stringify(domain),
    );
    ...
    fixtureAssert(
      g.cardinalityEnforcementPerformed === true &&
        g.cardinalityEnforcementResolved === true,
      JSON.stringify(g),
    );

    fixtureAssert(
      g.singletonSurvivorTreatedAsWinner === false &&
...
Deno.test(
  "v1.47 cardinality enforcement V1.5 one with three survivors is overflow",
  async () => {
    const result = await runCardinalityBehaviour(
      "one",
      "three",
    ...
    fixtureAssert(
      domain.survivingOccurrenceCount === 3 &&
        domain.minimum === 1 &&
        domain.maximum === 1 &&
        domain.enforcementState === "overflow" &&
        domain.cardinalitySatisfied === false,
      JSON.stringify(domain),
    );

    fixtureAssert(
      result.authority.overflowDomainCount === 1,
      JSON.stringify(result.authority),
    );
  },
);
...
Deno.test(
  "v1.47 cardinality enforcement V1.6 one_or_more with zero survivors is underflow",
  async () => {
    const result = await runCardinalityBehaviour(
      "one_or_more",
      "none",
...
Deno.test(
  "v1.47 cardinality enforcement V1.7 one_or_more with three survivors is satisfied",
  async () => {
    const result = await runCardinalityBehaviour(
      "one_or_more",
      "three",
...
Deno.test(
  "v1.47 cardinality enforcement V1.8 zero_or_more with zero survivors is satisfied",
  async () => {
    const result = await runCardinalityBehaviour(
      "zero_or_more",
      "none",
...
Deno.test(
  "v1.47 cardinality enforcement V1.9 zero_or_more with three survivors is satisfied",
  async () => {
    const result = await runCardinalityBehaviour(
      "zero_or_more",
      "three",
    ...
    fixtureAssert(
      result.authority.governance.unboundedMaximumTreatedAsNumericMaximum ===
        false,
      JSON.stringify(result.authority.governance),
    );
  },
);

Deno.test(
  "v1.47 cardinality enforcement V1.10 unsupported cardinality remains blocked upstream",
  async () => {
    const result = await runCardinalityBehaviour(
      "exactly_two",
      "one",
    ...
    fixtureAssert(
      result.blockingReasons.some(
        (reason) =>
          reason.includes(
            "current_snapshot_bound_cardinality_semantics:",
          ),
      ),
...
Deno.test(
  "v1.47 cardinality enforcement V1.11 violation is enforcement evidence not blocking failure or learner error",
  async () => {
    const underflow = await runCardinalityBehaviour(
      "one",
      "none",
    );

    const overflow = await runCardinalityBehaviour(
      "one",
      "three",
    );

    for (const result of [underflow, overflow]) {
      fixtureAssert(
        result.status === "ready",
        JSON.stringify(result),
      );
      ...
      fixtureAssert(
        g.cardinalityViolationIsBlockingFailure === false &&
...
Deno.test(
  "v1.47 cardinality enforcement V1.12 preserves exact semantic-domain lineage",
  async () => {
    const inputs = await makeV2Inputs({
      occurrenceMode: "one",
    });
    ...
    const source = semantics.authority.domainCardinalitySemantics[0]!;

    const derived = enforcement.authority.domainCardinalityEnforcement[0]!;

    fixtureAssert(
      enforcement.authority.sourceCardinalitySemanticsAuthorityId ===
        semantics.authority.authorityId,
      "semantics authority lineage lost",
    );

    fixtureAssert(
      derived.sourceCardinalitySemanticDomainId ===
          source.cardinalitySemanticDomainId &&
        derived.sourceFilteringDomainId ===
          source.sourceFilteringDomainId &&
...
Deno.test(
  "v1.47 cardinality enforcement V1.13 deterministic real authority derivation",
  async () => {
    const left = await runCardinalityBehaviour(
      "one",
      "three",
    );

    const right = await runCardinalityBehaviour(
      "one",
      "three",
    );
    ...
    fixtureAssert(
      ...
      "cardinality enforcement derivation is not deterministic",
    );
  },
);

console.log(
  "PASS: CURRENT snapshot-bound cardinality enforcement V1 Golden specification loaded.",
);
