...
Deno.test(
  "final runtime binding V1 exposes exact producer version and six-input API",
  () => {
    assertEquals(
      CANONICAL_RUNTIME_TOKEN_POS_CURRENT_SNAPSHOT_BOUND_FINAL_RUNTIME_BINDING_V1,
      "canonical-runtime-token-pos-current-snapshot-bound-final-runtime-binding-v1",
    );
...
Deno.test(
  "final runtime binding V1 reexecutes CLOSED projection and accepts no supplied wrapper",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: FINAL_SATISFIED_POS,
    });

    const result = requireFinalBindingReady(
      run.result,
    );
    ...
    fixtureAssert(
      g.closedBindingValueProjectionPublicDerivationReexecuted ===
          true &&
        g.suppliedBindingValueProjectionAcceptedAsProof ===
          false &&
        g.exactBindingValueProjectionAuthorityConsumed ===
          true,
      JSON.stringify(
        g,
      ),
...
Deno.test(
  "final runtime binding V1 one occurrence creates final one-member collection binding",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      domain.bindingState ===
        ...
        domain.boundOccurrenceCount ===
          1 &&
        domain.occurrenceBindingPerformed ===
          true &&
        domain.finalRuntimeOccurrenceBindingPerformed ===
          true,
      JSON.stringify(
        domain,
      ),
...
Deno.test(
  "final runtime binding V1 one_or_more preserves exact three-member collection",
  async () => {
    const run = await runFinalBindingFixture({
    ...
    const source = projection.authority
      .domainBindingValueProjection[0]!
      .bindingValue;

    const bound = result.authority
      .domainFinalRuntimeBinding[0]!
      .finalRuntimeBindingValue;
    fixtureAssert(
      source !== null &&
        bound !== null &&
        source.memberCount === 3 &&
        bound.memberCount === 3 &&
        source.members.length === 3 &&
        bound.members.length === 3,
      JSON.stringify({
        source,
        bound,
      }),
    ...
    if (source === null || bound === null) {
      return;
    }

    for (let i = 0; i < source.members.length; i++) {
      fixtureAssert(
        bound.members[i]!.bindingValueMemberId ===
            source.members[i]!.bindingValueMemberId &&
          bound.members[i]!.snapshotTokenOccurrenceIdentityId ===
            source.members[i]!.snapshotTokenOccurrenceIdentityId &&
          bound.members[i]!.sentenceTokenIndex ===
            source.members[i]!.sentenceTokenIndex,
        JSON.stringify({
...
Deno.test(
  "final runtime binding V1 zero_or_more zero creates legitimate final empty collection binding",
  async () => {
    const run = await runFinalBindingFixture({
    ...
    fixtureAssert(
      domain.bindingState ===
        ...
        domain.finalRuntimeBindingValue.memberCount ===
          0 &&
        domain.finalRuntimeBindingValue.isEmptyCollection ===
          true &&
        domain.boundOccurrenceCount ===
          0 &&
        domain.occurrenceBindingPerformed ===
        ...
        domain.finalRuntimeOccurrenceBindingPerformed ===
          false &&
        result.authority.emptyCollectionBoundDomainCount ===
          1 &&
        result.authority.governance
            .emptyCollectionTreatedAsOccurrenceBinding ===
          false,
      JSON.stringify(
        result.authority,
      ),
...
Deno.test(
  "final runtime binding V1 uncertain projection remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: FINAL_UNCERTAIN_POS,
    });
...
Deno.test(
  "final runtime binding V1 unavailable projection remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: FINAL_UNAVAILABLE_POS,
    });
...
Deno.test(
  "final runtime binding V1 blocked evidence projection remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: FINAL_BLOCKED_POS,
    });
...
Deno.test(
  "final runtime binding V1 underflow remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "none",
...
Deno.test(
  "final runtime binding V1 overflow remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "three",
...
Deno.test(
  "final runtime binding V1 proven false occurrence remains not bound",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
      pos: FINAL_REJECTED_POS,
    });
...
Deno.test(
  "final runtime binding V1 preserves exact projection authority and binding lineage",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      result.authority
            .sourceBindingValueProjectionAuthorityId ===
          projection.authority.authorityId &&
        bound.sourceBindingValueProjectionDomainId ===
          source.bindingValueProjectionDomainId &&
        bound.bindingDefinitionAuthorityId ===
          source.bindingDefinitionAuthorityId &&
        bound.expectedSiteAuthorityId ===
          source.expectedSiteAuthorityId &&
        bound.referencedBindingName ===
          source.referencedBindingName &&
        bound.snapshotIdentityId ===
          source.snapshotIdentityId &&
        bound.snapshotSentenceOccurrenceIdentityId ===
...
Deno.test(
  "final runtime binding V1 singleton collection remains collection and never winner",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      domain.finalRuntimeBindingValue !==
          null &&
        domain.finalRuntimeBindingValue.valueKind ===
          "ordered_snapshot_token_occurrence_collection" &&
        domain.finalRuntimeBindingValue.memberCount ===
          1 &&
        g.singletonCollectionCollapsedToScalar ===
          false &&
        g.singletonCollectionTreatedAsWinner ===
          false,
      JSON.stringify({
        domain,
        g,
...
Deno.test(
  "final runtime binding V1 owns occurrence binding but not runtime execution truth or WHERE",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "one",
      occurrenceMode: "one",
    ...
    fixtureAssert(
      g.finalRuntimeBindingResolutionPerformed ===
          true &&
        g.finalRuntimeBindingResolved ===
          true &&
...
Deno.test(
  "final runtime binding V1 empty collection is final binding without occurrence-binding claim",
  async () => {
    const run = await runFinalBindingFixture({
      cardinality: "zero_or_more",
      occurrenceMode: "none",
    ...
    fixtureAssert(
      result.authority.boundDomainCount ===
          1 &&
        result.authority.boundOccurrenceCount ===
          0 &&
        g.finalRuntimeBindingResolved ===
          true &&
        g.occurrenceBindingPerformed ===
        ...
        g.finalRuntimeOccurrenceBindingPerformed ===
          false &&
        g.emptyCollectionTreatedAsOccurrenceBinding ===
          false,
      JSON.stringify(
        result.authority,
      ),
...
Deno.test(
  "final runtime binding V1 unsupported cardinality remains blocked upstream",
  async () => {
    const run = await runFinalBindingFixture({
    ...
    fixtureAssert(
      run.result.authority ===
        ...
        run.result.blockingReasons.some(
          (reason) =>
            reason.includes(
              "current_snapshot_bound_binding_value_projection:",
            ),
        ),
      JSON.stringify(
        run.result,
...
Deno.test(
  "final runtime binding V1 derivation is deterministic and source inputs remain immutable",
  async () => {
    const inputs = await makeV2Inputs({
      occurrenceMode: "three",
