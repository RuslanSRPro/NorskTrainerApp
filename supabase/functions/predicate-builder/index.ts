// supabase/functions/predicate-builder/index.ts
//
// Norsk Trainer
// Predicate Builder v1.4
//
// Projects ranked tokens and a resolved Construction Model into PredicateFrame objects and Sentence Model v1.
// It does not tokenize, resolve surfaces, match grammar rules, resolve construction conflicts,
// or infer subjects/objects/clauses that have not yet been detected.
...
type PredicateFrame = {
  ...
  /**
   * Stable provenance back to the resolved Construction Model.
   * One PredicateFrame is projected from one root construction tree.
   */
  /**
   * Canonical construction provenance.
   * construction_ids contains the root first, followed by descendants.
   */
...
const PREDICATE_BUILDER_VERSION = 'predicate-builder-v1.5';
const SENTENCE_MODEL_VERSION = 'sentence-model-v1.2';
...
function constructionTreeDepth(
  root: GrammarConstruction,
  model: ConstructionModel | undefined,
): number {
  if (!model) return 0;

  const rootId = constructionId(root);

  if (model.construction_tree?.nodes?.length) {
    const nodesById = new Map(
      model.construction_tree.nodes.map((node) => [
        node.construction_id,
        node,
      ]),
    );

    if (!nodesById.has(rootId)) return 0;
    ...
    const walk = (
      ...
      for (const childId of node.child_construction_ids ?? []) {
        walk(childId, relativeDepth + 1);
      }
    };
...
function createPredicate(
  ...
  const semanticConstruction = descendants.length > 0
    ...
    : construction;
  // The semantic head belongs to the deepest semantic construction,
  // while finiteness belongs to the root construction.
  const semanticRoles = extractDeclaredSlotRoles(
    semanticConstruction.result ?? {},
  );
  const rootRoles = extractDeclaredSlotRoles(
    construction.result ?? {},
  );

  const headSlot = resolveHeadSlot(
  ...
  if (finiteMember) {
    const correspondingMember = members.find(
      (member) =>
        member.token_index === finiteMember.token_index &&
        member.slot === finiteMember.slot,
    );

    if (!correspondingMember) {
      diagnostics.push({
        severity: 'error',
        code: 'finite_member_not_in_predicate_members',
        message:
          `Finite member for ${construction.rule_code} does not correspond to any predicate member.`,
        token_index: finiteMember.token_index,
        rule_code: construction.rule_code,
        metadata: {
          finite_member_role: finiteMember.role,
          finite_member_slot: finiteMember.slot,
          finite_member_token_index: finiteMember.token_index,
        },
      });
    } else if (correspondingMember.role !== finiteMember.role) {
      diagnostics.push({
        severity: 'error',
        code: 'predicate_role_invariant_violation',
        message:
          `Finite member role "${finiteMember.role}" is inconsistent with members[] role "${correspondingMember.role}".`,
        token_index: finiteMember.token_index,
        rule_code: construction.rule_code,
        metadata: {
          finite_member_role: finiteMember.role,
          member_role: correspondingMember.role,
          slot: finiteMember.slot,
          token_index: finiteMember.token_index,
        },
      });
    }
  ...
  const correspondingHeadMember = members.find(
    (member) =>
      member.token_index === headBinding.token_index &&
      member.slot === headBinding.slot,
  );

  const resolvedHeadRole =
    semanticRoles.get(headSlot) ?? 'predicate_head';

  if (!correspondingHeadMember) {
    diagnostics.push({
      severity: 'error',
      code: 'predicate_head_not_in_members',
      message:
        `Predicate head for ${construction.rule_code} does not correspond to any predicate member.`,
      token_index: headBinding.token_index,
      rule_code: construction.rule_code,
      metadata: {
        head_role: resolvedHeadRole,
        head_slot: headBinding.slot,
        head_token_index: headBinding.token_index,
      },
    });
  } else if (correspondingHeadMember.role !== resolvedHeadRole) {
    diagnostics.push({
      severity: 'error',
      code: 'predicate_head_role_invariant_violation',
      message:
        `Predicate head role "${resolvedHeadRole}" is inconsistent with members[] role "${correspondingHeadMember.role}".`,
      token_index: headBinding.token_index,
      rule_code: construction.rule_code,
      metadata: {
        head_role: resolvedHeadRole,
        member_role: correspondingHeadMember.role,
        slot: headBinding.slot,
        token_index: headBinding.token_index,
      },
    });
  }

  const constructionTreeRootId = constructionId(construction);

  // Canonical provenance order:
  // root first, then descendants in deterministic traversal order.
  const constructionIds = [
    constructionTreeRootId,
    ...descendants.map(constructionId),
  ];

  if (new Set(constructionIds).size !== constructionIds.length) {
    diagnostics.push({
      severity: 'error',
      code: 'duplicate_construction_provenance_id',
      message:
        `Predicate ${construction.rule_code} contains duplicate construction provenance IDs.`,
      rule_code: construction.rule_code,
      metadata: {
        construction_ids: constructionIds,
      },
    });
  }

  const treeDepth = constructionTreeDepth(
    construction,
    constructionModel,
  );

  const semanticConstructionId =
    constructionId(semanticConstruction);

  if (constructionIds[0] !== constructionTreeRootId) {
    diagnostics.push({
      severity: 'error',
      code: 'predicate_provenance_root_mismatch',
      message:
        `Predicate ${construction.rule_code} has a construction_ids root mismatch.`,
      rule_code: construction.rule_code,
      metadata: {
        construction_tree_root_id: constructionTreeRootId,
        first_construction_id: constructionIds[0] ?? null,
      },
    });
  }

  if (!constructionIds.includes(semanticConstructionId)) {
    diagnostics.push({
      severity: 'error',
      code: 'predicate_semantic_construction_missing',
      message:
        `Predicate ${construction.rule_code} semantic construction is absent from construction_ids.`,
      rule_code: construction.rule_code,
      metadata: {
        semantic_construction_id: semanticConstructionId,
        construction_ids: constructionIds,
      },
    });
  }
  ...
  if (!Number.isInteger(treeDepth) || treeDepth < 0) {
    diagnostics.push({
      severity: 'error',
      code: 'predicate_construction_tree_depth_invalid',
      message:
        `Predicate ${construction.rule_code} has an invalid construction tree depth.`,
      rule_code: construction.rule_code,
      metadata: {
        construction_tree_depth: treeDepth,
      },
    });
  }

  const composedStart = Math.min(
    ...allConstructions.map((item) => item.token_start),
  );
  const composedEnd = Math.max(
    ...allConstructions.map((item) => item.token_end),
  );
  ...
  return {
    ...
    // Composition/provenance metadata is canonical at PredicateFrame top level.
    source_result: {
      ...(construction.result ?? {}),
    },
    ...
    root_construction_type: construction.construction_type,
    semantic_construction_id: semanticConstructionId,
    semantic_construction_type:
      semanticConstruction.construction_type,
    construction_tree_depth: treeDepth,
  };
}
...
function buildSentenceModel(request: InterpretationRequest) {
  ...
  const tokenModels = request.tokens
    .map((token) => {
      ...
      const selectedSourceCandidate = selected
        ? token.candidates.find(
            (candidate) =>
              candidate.lexeme_id === selected.lexeme_id &&
              candidate.lemma === selected.lemma &&
              candidate.pos === selected.pos,
          ) ?? null
        : null;
      ...
      return {
        ...
        /**
         * Lexical enrichment is deliberately separate from selected_candidate.
         * selected_candidate remains the stable candidate-selection contract.
         */
        lexical_analysis:
          selectedSourceCandidate?.lexical_analysis
            ? structuredClone(selectedSourceCandidate.lexical_analysis)
            : null,
  ...
  return {
    ...
    predicate_contract_version: PREDICATE_CONTRACT_VERSION,
  ...
  try {
    if (request.method !== 'POST') {
      return jsonResponse(
        {
          ok: false,
          builder_version: PREDICATE_BUILDER_VERSION,
          predicate_builder_version: PREDICATE_BUILDER_VERSION,
          predicate_contract_version: PREDICATE_CONTRACT_VERSION,
          // Deprecated compatibility alias. Remove in v2.0.
          interpreter_version: PREDICATE_BUILDER_VERSION,
    ...
    console.error(
      validation
        ? '[PREDICATE BUILDER REQUEST ERROR]'
        : '[PREDICATE BUILDER ERROR]',
      error,
    );
