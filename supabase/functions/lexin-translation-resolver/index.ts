...
function normalizeLexinUkTranslations(text: string): string[] {
  const raw = text.trim();
  if (!raw) return [];

  // Lexin часто даёт украинские пары вида:
  // "прийняти|приймати"
  // "прийняти, зібрати|приймати, збирати"
  // Берём правую часть = несовершенный вид.
  const preferredSide = raw.includes('|')
    ? raw.split('|').map((p) => p.trim()).filter(Boolean).at(-1) ?? raw
    : raw;

  // Только основные переводы из Ukr-lem разбиваем на отдельные значения.
  // Ukr-def не трогаем и не смешиваем с primary.
  const parts = preferredSide
    .split(/[,;]/)
    .map((p) => p.trim())
    .filter(Boolean);
...
async function fetchLexinTranslations(word: string): Promise<{
  ...
  if (!response.ok) {
    throw new Error(`Lexin HTTP ${response.status} for "${word}"`);
  }
  ...
  for (const group of groups) {
    ...
    // Не импортируем переводы из чужих групп / других лемм.
    if (!isExactMatch(group, word)) continue;

    for (const e of group) {
      if (e.type === 'Ukr-lem' && e.text?.trim()) {
        ukrLem.push(...normalizeLexinUkTranslations(e.text));
      }

      if (e.type === 'Ukr-def' && e.text?.trim()) {
        ukrDef.push(e.text.trim());
      }

      if (e.type === 'B-lem' && e.text?.trim()) {
        engLem.push(e.text.trim());
      }
    }
  }
...
async function loadGlossCandidates(supabase: any, limit: number) {
  ...
  for (const c of data ?? []) {
    ...
    if (!uniqueTerms.has(term)) {
      uniqueTerms.set(term, {
        ids: [],
        sourceExpressionId: c.source_expression_id ?? null,
      });
    }
...
serve(async (req) => {
  try {
    ...
    for (const [term, { ids }] of toResolve) {
      try {
        ...
        resolved.push({
          ...
          note:
            !targetLexemeId && lexinResult.found
