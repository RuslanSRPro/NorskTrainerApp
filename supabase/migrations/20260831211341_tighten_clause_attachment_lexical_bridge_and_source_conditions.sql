...
begin
  ...
  loop
    for ex in select jsonb_array_elements_text(coalesce(c.details->'examples','[]'::jsonb)) loop
      ...
      select count(*),(min(l.id::text))::uuid into v_match_count,v_lexeme_id from public.lexemes l where l.pos='verb' and lower(l.lemma)=lower(ex);
      if v_match_count<>1 then
        v_unmapped:=v_unmapped||jsonb_build_array(jsonb_build_object('source_surface',ex,'candidate_code',c.candidate_code,'reason',case when exists(select 1 from public.lexeme_form_variants f join public.lexemes l on l.id=f.lexeme_id where l.pos='verb' and f.is_accepted is true and lower(f.value)=lower(ex)) then 'variant_or_cross_standard_bridge_requires_sense_resolution' else 'no_exact_verified_lemma_bridge' end,'exact_lemma_match_count',v_match_count));
        continue;
      end if;
      v_prov:=jsonb_build_array(jsonb_build_object('candidate_id',c.id,'candidate_code',c.candidate_code,'source_section',c.source_section,'title',c.title,'source_excerpt',c.source_excerpt,'verification_status','source_verified','source_surface',ex,'runtime_lexeme_id',v_lexeme_id,'lexical_bridge','exact_lemma'));
      for comp in select jsonb_array_elements_text(coalesce(c.details->'complements','[]'::jsonb)) loop
        ...
        values(v_release_id,v_code,'predicate_complement_frame','lexeme',v_lexeme_id::text,'licenses_complement','construction_family',comp,'license','categorical',jsonb_build_object('predicate_class',c.details->>'class','source_requires',coalesce(c.details->'requires','[]'::jsonb)),jsonb_build_object('source_surface',ex,'source_class',c.details->>'class','runtime_lemma',(select lemma from public.lexemes where id=v_lexeme_id),'bridge_method','exact_lemma'),v_prov)
      ...
      if c.details ? 'excluded' then
        ...
        insert into public.grammar_runtime_constraint_facts_v1(release_id,constraint_code,fact_family,subject_type,subject_key,relation,object_type,object_key,polarity,strength,conditions,payload,provenance)
        values(v_release_id,v_code,'predicate_complement_frame','lexeme',v_lexeme_id::text,'licenses_complement','construction_family',comp,'exclude','categorical',jsonb_build_object('predicate_class',c.details->>'class','source_requires',coalesce(c.details->'requires','[]'::jsonb)),jsonb_build_object('source_surface',ex,'source_class',c.details->>'class','runtime_lemma',(select lemma from public.lexemes where id=v_lexeme_id),'bridge_method','exact_lemma'),v_prov)
        on conflict (release_id,constraint_code) do update set conditions=excluded.conditions,payload=excluded.payload,provenance=excluded.provenance,is_enabled=true;
        v_inserted:=v_inserted+1;
      end if;
    end loop;
