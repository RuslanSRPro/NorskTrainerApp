...
declare v_selected text; v_pos text; v_match boolean:=false;
begin
 select lower(x->>'selected_lemma') into v_selected from jsonb_array_elements(coalesce(p_analysis#>'{language_graph,morphology_v1}','[]'::jsonb)) x
 ...
 select x->>'selected_grammar_pos' into v_pos from jsonb_array_elements(coalesce(p_analysis#>'{language_graph,local_pos_v1}','[]'::jsonb)) x where nullif(x->>'token_index','')::int=p_token_index limit 1;
 if v_pos<>'verb' then return false; end if;
 select exists(
...
begin
 ...
 return jsonb_build_object('version','multiword-function-expression-v1','status','ready','release_code',p_release_code,'scope','contiguous_mwe_v1','resolved_expressions',resolved,'candidate_expressions',candidates,'blocked_or_deferred',blocked,'summary',jsonb_build_object('inventory_fact_count',scanned,'matched_occurrence_count',matched,'resolved_expression_count',jsonb_array_length(resolved),'candidate_expression_count',jsonb_array_length(candidates),'blocked_or_deferred_count',jsonb_array_length(blocked),'source_graph_runtime_reads',0,'learner_error_claims',0));
end;
$$;
