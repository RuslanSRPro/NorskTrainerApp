create or replace function public.promote_sentence_model_release_to_shadow_v2(p_batch_id uuid,p_release_code text default 'runtime-structural-v1.15')
returns jsonb
language plpgsql
security invoker
...
declare
 ...
 g jsonb; vals jsonb; ev text; expected_count int; pass_count int; fail_count int;
...
begin
 ...
 g:=public.run_sentence_model_golden_v2(p_release_code);
 if (g->>'total')::int<>36 or (g->>'passed')::int<>36 or (g->>'failed')::int<>0 then raise exception 'Sentence Model Golden gate failed: %',g; end if;
 ...
 update public.grammar_runtime_releases set status='shadow',metadata=metadata||jsonb_build_object(
  ...
  'promotion_gate_version','sentence-model-v2-shadow-gate-v1','promotion_sentence_model_gate',36,
  'promotion_interpretation_gate',33,'promotion_grammar_validation_gate',26,'promotion_dependency_build_v2_gate',24,
  'promotion_clause_build_gate',24,'promotion_predicate_build_gate',24,'promotion_construction_resolution_gate',24,
  ...
  'promotion_dependency_v1_gate',7,'promotion_tokenizer_integration_gate',7,'promotion_segmentation_integration_gate',6,
  'promotion_single_sentence_parity',34,'promotion_machine_expectations',18,'promotion_child_only_rules',0,
  'immutable_hash_gate',jsonb_build_object('legacy_tokenizer',legacy_hash,'structural_core_v1',core_hash,'tokenizer_v2',tok_hash)
 ) where id=rid;

 update public.grammar_shadow_v2_batches set status='reviewed',metadata=metadata||jsonb_build_object(
  'promotion','golden_to_shadow','promotion_gate_version','sentence-model-v2-shadow-gate-v1',
  'causal_review',jsonb_build_object('reviewed',true,'capability','Sentence Model V2','comparison',parent||' -> '||p_release_code,'strict_parity_cases',34,'machine_expectations_passed',18,'interpretation','Sentence Model V2 is a canonical additive projection; all upstream analysis remains identical.')
 ),updated_at=now() where id=p_batch_id;

 return jsonb_build_object('release_code',p_release_code,'release_status','shadow','parent_release',parent,'batch_id',p_batch_id,'batch_status','reviewed',
  'sentence_model_golden',36,'interpretation_golden',33,'grammar_validation_golden',26,'dependency_build_v2_golden',24,'clause_build_golden',24,
  'predicate_build_golden',24,'construction_resolution_golden',24,'construction_recognition_golden',23,'structural_pos_golden',18,'phrase_build_golden',18,
  ...
  'single_sentence_parity',34,'machine_expectations',18,'active_nrg_rules',active,'child_only_rules',child_rules,'promotion_gate_version','sentence-model-v2-shadow-gate-v1');
end;
$function$;
