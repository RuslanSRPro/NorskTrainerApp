do $migration$
declare m record; gr record; base_pattern jsonb; enrichment jsonb; outputs jsonb; k text;
begin
 for m in select * from public.grammar_runtime_manifests order by code loop
   if exists(select 1 from public.grammar_runtime_compiler_plans_v1 p where p.manifest_id=m.id and p.compiler_contract='grammar-runtime-compiler-v1' and p.status='validated') then
     continue;
   end if;
   outputs:='[]'::jsonb;
   for gr in select * from public.grammar_rules where runtime_manifest_id=m.id order by code loop
     base_pattern:=public.runtime_compiler_pattern_v1(m.ir_spec,gr.pattern_type,null);
     enrichment:=gr.pattern;
     for k in select key from jsonb_each(base_pattern) loop
       enrichment:=enrichment-k;
     end loop;
     outputs:=outputs||jsonb_build_array(jsonb_build_object(
       'pattern_type',gr.pattern_type,
       'rule_type',gr.rule_type,
       'code_override',gr.code,
       'compile_role','primary',
       'pattern_enrichment',enrichment,
       'bootstrap_source','historical_compiled_rule'
     ));
   end loop;
   if jsonb_array_length(outputs)>0 then
     insert into public.grammar_runtime_compiler_plans_v1(manifest_id,output_plan,notes)
     values(m.id,jsonb_build_object('outputs',outputs),'Bootstrap captures historical compiler enrichment as explicit build metadata; validated manifest semantics unchanged.');
   end if;
 end loop;
end;
$migration$;
...
as $function$
declare m record; v_plan jsonb; o jsonb; v_outputs jsonb:='[]'::jsonb; v_pattern_type text; v_rule_type text; v_suffix text; v_code text; v_branch text; v_pattern jsonb; v_actions jsonb; v_result jsonb; v_hash text; v_strategy text; v_targets jsonb; v_enrichment jsonb;
begin
 ...
 if v_plan is null and v_strategy='single_rule' then
   v_plan:=jsonb_build_object('outputs',jsonb_build_array(jsonb_build_object('pattern_type',v_targets->>0)));
 ...
 for o in select value from jsonb_array_elements(v_plan->'outputs') loop
   ...
   v_enrichment:=coalesce(o->'pattern_enrichment','{}'::jsonb);
   v_pattern:=v_pattern||v_enrichment;
