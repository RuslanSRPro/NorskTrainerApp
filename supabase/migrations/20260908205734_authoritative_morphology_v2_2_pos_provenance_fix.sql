-- D10 / AUTHORITATIVE MORPHOLOGY V2.2 POS-SCOPED PROVENANCE
-- Correct the V2.2 publisher without editing the applied source-catalog
-- migration. Exact lookup may return homonymous articles for another POS.
-- Exact dictionary lookup may return homonymous articles for another POS.
-- The publisher must persist and bind only articles contributing validated
-- paradigms to the accepted display projection for the target lexeme POS.
...
begin
  ...
  -- Persist only source articles that contributed a validated paradigm for
  -- the requested POS. Exact lookup may return homonymous articles for other
  -- parts of speech; those articles must not be stored or bound as provenance
  -- for this lexeme snapshot.
  for v_source_article in
    select source_article.value
    from jsonb_array_elements(v_lookup -> 'articles')
      as source_article(value)
    where source_article.value ->> 'dictionaryCode' = 'bm'
      and exists (
        select 1
        from unnest(v_article_ids) as accepted_article(article_id)
        where accepted_article.article_id::text =
          source_article.value ->> 'articleId'
      )
  loop
...
comment on function public.publish_authoritative_morphology_snapshot_v2(
  ...
  'Service-role-only atomic D10 publisher; stores and binds only source articles contributing validated paradigms for the target lexeme POS.';
