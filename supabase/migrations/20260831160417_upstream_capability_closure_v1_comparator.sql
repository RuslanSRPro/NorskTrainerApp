create or replace function public.run_upstream_capability_closure_shadow_comparator_v1(p_release_code text default 'runtime-structural-v1.21',p_parent_release_code text default 'runtime-structural-v1.20',p_corpus_version text default 'shadow-corpus-v2.0')
returns uuid
language plpgsql
security invoker
...
begin
 ...
 values(child_id,p_corpus_version,p_parent_release_code,'grammar-structural-shadow-v20','grammar-structural-shadow-v21',public.grammar_shadow_v2_legacy_rule_checksum(),'running',expected,jsonb_build_object('comparison_contract','grammar-shadow-comparison-v2.2','evaluation_contract','grammar-shadow-comparator-v2.2','comparison_mode','parent_child_upstream_capability_closure_causal','parent_release',p_parent_release_code,'child_release',p_release_code,'single_capability','Upstream Capability Closure V1','legacy_is_oracle',false,'child_projection','common semantic projection excludes agreement_controller_bridge_v1 and clause_field_model_v1'),clock_timestamp()) returning id into batch;
 for c in select id,code,input_text from public.grammar_shadow_v2_corpus_cases where corpus_version=p_corpus_version and is_active order by code loop
  begin
   pd:=public.analyze_text_structural_shadow_v20(c.input_text,p_parent_release_code); cd:=public.analyze_text_structural_shadow_v21(c.input_text,p_release_code);
   pa:=coalesce(pd#>'{document_graph,sentences,0,analysis}','{}'::jsonb); ca:=coalesce(cd#>'{document_graph,sentences,0,analysis}','{}'::jsonb);
   ca:=jsonb_set(ca,'{language_graph}',coalesce(ca->'language_graph','{}'::jsonb)-'agreement_controller_bridge_v1'-'clause_field_model_v1',true);
   ...
   values(batch,c.id,c.code,c.input_text,'completed',cmp->>'classification',coalesce(cmp->'labels','[]'::jsonb),pa,ca,pp,cp,cmp,'grammar-structural-shadow-v20','grammar-structural-shadow-v21');
  exception when others then
   ...
   values(batch,c.id,c.code,c.input_text,'error',null,'[]','{}','{}','{}','{}','{}','grammar-structural-shadow-v20','grammar-structural-shadow-v21',sqlerrm);
  end;
 end loop;
