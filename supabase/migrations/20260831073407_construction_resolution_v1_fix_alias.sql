...
begin
  ...
  for v_overlap_item in select x from jsonb_array_elements(v_overlaps) x loop
    select x into v_left from jsonb_array_elements(v_cons) x where x->>'id'=v_overlap_item->>'left_id' limit 1;
    select x into v_right from jsonb_array_elements(v_cons) x where x->>'id'=v_overlap_item->>'right_id' limit 1;
    ...
    v_relations:=v_relations||jsonb_build_array(jsonb_build_object(
      'left_id',v_overlap_item->>'left_id','right_id',v_overlap_item->>'right_id','relation',v_relation,'reason_code',v_reason,
  ...
  for g in
    ...
    edges as (
      select ov->>'left_id' a,ov->>'right_id' b from jsonb_array_elements(v_overlaps) ov
      union all
      select ov->>'right_id',ov->>'left_id' from jsonb_array_elements(v_overlaps) ov
    ),
    reach(root,id) as (
    ...
    select component_key,
           jsonb_agg(q.c order by (q.c->>'span_start')::int,(q.c->>'span_end')::int,q.c->>'id') constructions,
           min((q.c->>'span_start')::int) span_start,
           max((q.c->>'span_end')::int) span_end
    from comp cp join lateral (
      select x c from jsonb_array_elements(v_cons) x where x->>'id'=cp.id
    ) q on true
    group by component_key
    order by min((q.c->>'span_start')::int),max((q.c->>'span_end')::int)
  loop
  ...
  for b in select x from jsonb_array_elements(v_blocked) x loop
    if not exists(
      select 1 from jsonb_array_elements(v_cons) cx
      where (cx->>'span_start') is not null and (cx->>'span_end') is not null
        and (cx->>'span_start')::int <= (b->>'span_end')::int
        and (cx->>'span_end')::int >= (b->>'span_start')::int
    ) then
      v_groups:=v_groups||jsonb_build_array(jsonb_build_object(
