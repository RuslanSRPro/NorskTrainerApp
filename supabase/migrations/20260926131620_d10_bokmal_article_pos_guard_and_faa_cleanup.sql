-- D10: honor registered Bokmål Ordbokene article POS at promotion.
  ...
  blocked_pos_occurrences as (
    select
      ...
      case
        when coalesce((rs.verification_evidence->'Ordbokene'->>'registered_entry')::boolean, false)
          and rs.verification_evidence->'Ordbokene'->'evidence'->'raw_preview'->>'dictionary_code' = 'bm'
          and nullif(rs.verification_evidence->'Ordbokene'->'evidence'->'raw_preview'->>'pos', '') is not null
          and rs.verification_evidence->'Ordbokene'->'evidence'->'raw_preview'->>'pos' <> rs.pos
...
-- Retract exactly the manually inserted Bokmål adjective identity. Its one
-- learning_progress row is deleted by FK CASCADE; historical processing rows
-- retain their source evidence but no longer claim a valid promotion.
do $cleanup$
declare
  v_bad_id constant uuid := '585a8319-8613-4994-872c-7d3cbf0da850';
  v_verb_id constant uuid := 'e07c93b8-0b08-4484-8801-e5fcbef92e2e';
  v_deleted integer;
begin
  if (select count(*) from public.lexemes where id = v_bad_id and lemma = 'få' and pos = 'adjective') <> 1
     or not exists (select 1 from public.lexemes where id = v_verb_id and lemma = 'få' and pos = 'verb') then
    raise exception 'D10 FAA cleanup precondition failed: expected adjective and verb identities';
  end if;
  if exists (select 1 from public.lexeme_compound_components where component_lexeme_id = v_bad_id)
     or exists (select 1 from public.lexin_gloss_candidates where target_lexeme_id = v_bad_id)
     or exists (select 1 from public.reading_session_words where lexeme_id = v_bad_id) then
    raise exception 'D10 FAA cleanup blocked by restrictive references';
  end if;
  if exists (select 1 from public.expression_catalog where lexeme_id = v_bad_id)
     or exists (select 1 from private.lexeme360_expression_roots_v1 where root_lexeme_id = v_bad_id) then
    raise exception 'D10 FAA cleanup blocked by expression or root reference';
  end if;

  update public.lexeme_processing_items
  set current_stage = 'admission_gate',
      result_summary = coalesce(result_summary, '{}'::jsonb)
        || jsonb_build_object(
          'promotion_status', 'retracted_wrong_pos',
          'admission_reason', 'CONFIRMED_ORDBOKENE_POS_CONFLICT',
          'retraction_version', 'd10_faa_bokmal_pos_guard_v1'
        )
  where lexeme_id = v_bad_id;

  delete from public.lexemes
  where id = v_bad_id and lemma = 'få' and pos = 'adjective';
  get diagnostics v_deleted = row_count;
  if v_deleted <> 1 then
    raise exception 'D10 FAA cleanup deleted % lexemes, expected 1', v_deleted;
  end if;
end;
$cleanup$;
