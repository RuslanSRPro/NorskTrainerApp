-- Заменяет 'source_found' и 'unknown' на 'ai_candidate' в verification_tier.
-- Эти два значения не входят в 6-уровневый клиентский enum
-- (dictionary_entry / dictionary_match / usage_evidence / component_match /
-- ai_candidate / unverified) и на клиенте откатывались на ai_candidate сами,
-- но негласно — лучше писать это явно на стороне SQL.
-- Источник находки: architecture-audit-full.md, раздел 27.
      ...
      jsonb_object_agg(
        ...
        jsonb_build_object(
          ...
          'authoritative_relations',
            coalesce(sc.authoritative_relations, '[]'::jsonb),
    ...
    set
      ...
      result_summary =
        ...
        || jsonb_build_object(
          ...
          'promotion_version', 'expression_promotion_v4_canonical_tier_fallback'
        ),
      updated_at = now()
    where i.job_id = p_job_id
    ...
    set
      ...
      result_summary =
        ...
        || jsonb_build_object(
          ...
          'lexeme_id', lm.lexeme_id,
          'promotion_version', 'verification_promotion_v5_canonical_tier_fallback'
        ),
      updated_at = now()
    from lexeme_matches lm
...
-- Бэкафилл: исправить уже накопленные строки со старыми значениями.
-- Перед запуском в проде стоит свериться с числом из проверки №5
-- (раздел "PowerShell + SQL" предыдущей переписки), сколько строк затронется.
update public.lexemes
set verification_tier = 'ai_candidate'
where verification_tier in ('source_found', 'unknown');
