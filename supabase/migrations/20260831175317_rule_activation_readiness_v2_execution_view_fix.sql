...
begin
 ...
 select count(*) into manifest_count from public.grammar_runtime_manifests m
 where m.authoring_status='validated' and (m.primary_candidate_id=p_candidate_id or exists(select 1 from public.grammar_runtime_manifest_sources ms where ms.manifest_id=m.id and ms.candidate_id=p_candidate_id));
 ...
 return jsonb_build_object(
  'version','rule-activation-readiness-v2','candidate_id',c.id,'candidate_code',e.candidate_code,
  'release_code',p_release_code,'readiness_state',state,'runtime_eligible',coalesce(e.runtime_eligible,false),
