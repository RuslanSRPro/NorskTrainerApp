create or replace function public.run_pedagogical_projection_shadow_comparator_v1(
  p_release_code text default 'runtime-structural-v1.16',
  p_parent_release_code text default 'runtime-structural-v1.15',
...
as $function$
declare
  child_id uuid; child_status text; parent_status text; expected int; batch uuid; c record;
  pd jsonb; cd jsonb; pa jsonb; ca jsonb; pp jsonb; cp jsonb; cmp jsonb;
begin
  ...
  insert into public.grammar_shadow_v2_batches(
    ...
    child_id,p_corpus_version,p_parent_release_code,'grammar-structural-shadow-v15','grammar-structural-shadow-v16',
    public.grammar_shadow_v2_legacy_rule_checksum(),'running',expected,
    jsonb_build_object(
      'comparison_contract','grammar-shadow-comparison-v2.2',
      'evaluation_contract','grammar-shadow-comparator-v2.2',
      'comparison_mode','parent_child_pedagogical_projection_causal',
      'parent_release',p_parent_release_code,
      'child_release',p_release_code,
      'single_capability','Pedagogical Projection V1',
      'legacy_is_oracle',false,
      'child_projection','common semantic projection excludes pedagogical_projection_v1'
    ),clock_timestamp()
  ...
  for c in select id,code,input_text from public.grammar_shadow_v2_corpus_cases where corpus_version=p_corpus_version and is_active order by code loop
    begin
      pd:=public.analyze_text_structural_shadow_v15(c.input_text,p_parent_release_code);
      cd:=public.analyze_text_structural_shadow_v16(c.input_text,p_release_code);
      pa:=coalesce(pd#>'{document_graph,sentences,0,analysis}','{}'::jsonb);
      ca:=coalesce(cd#>'{document_graph,sentences,0,analysis}','{}'::jsonb);
      ca:=jsonb_set(ca,'{language_graph}',coalesce(ca->'language_graph','{}'::jsonb)-'pedagogical_projection_v1',true);
      ...
      insert into public.grammar_shadow_v2_comparisons(
        ...
        baseline_result,shadow_result,baseline_projection,shadow_projection,comparison,baseline_version,shadow_version
      ) values(
        batch,c.id,c.code,c.input_text,'completed',cmp->>'classification',coalesce(cmp->'labels','[]'::jsonb),
        pa,ca,pp,cp,cmp,'grammar-structural-shadow-v15','grammar-structural-shadow-v16'
      );
    exception when others then
      insert into public.grammar_shadow_v2_comparisons(
        batch_id,case_id,case_code,input_text,execution_status,classification,labels,
        baseline_result,shadow_result,baseline_projection,shadow_projection,comparison,baseline_version,shadow_version,error_message
      ) values(
        batch,c.id,c.code,c.input_text,'error',null,'[]','{}','{}','{}','{}','{}',
        'grammar-structural-shadow-v15','grammar-structural-shadow-v16',sqlerrm
      );
    end;
  end loop;
