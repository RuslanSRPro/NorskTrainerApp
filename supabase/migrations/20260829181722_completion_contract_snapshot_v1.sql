-- D09-C4 / Package 1
-- Read-only, versioned evidence snapshot for completion-contract shadow evaluation.
-- This function creates no learner-facing state and performs no healing or writes.
...
begin
  ...
  v_execution_state := case
    when v_job.status = 'needs_manual_review'
...
'Read-only, stable, fail-closed evidence snapshot for completion-contract/v1 shadow evaluation. Service-role only.';
