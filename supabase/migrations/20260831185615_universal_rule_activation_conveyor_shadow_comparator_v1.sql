create or replace function public.run_universal_rule_activation_conveyor_shadow_comparator_v1(p_release_code text default 'runtime-structural-v1.30',p_parent_release_code text default 'runtime-structural-v1.29',p_corpus_version text default 'shadow-corpus-v2.0')
returns uuid
language plpgsql
security invoker
...
begin
 ...
 values(child_id,p_corpus_version,p_parent_release_code,'grammar-structural-shadow-v29','grammar-structural-shadow-v30',public.grammar_shadow_v2_legacy_rule_checksum(),'running',expected,
 jsonb_build_object('comparison_mode','parent_child_universal_activation_conveyor_v1','parent_release',p_parent_release_code,'child_release',p_release_code,'single_capability','Universal Rule Activation Conveyor V1'),clock_timestamp()) returning id into batch;
 for c in select id,code,input_text from public.grammar_shadow_v2_corpus_cases where corpus_version=p_corpus_version and is_active order by code loop
  begin
   pd:=public.analyze_text_structural_shadow_v29(c.input_text,p_parent_release_code); cd:=public.analyze_text_structural_shadow_v30(c.input_text,p_release_code);
   ...
   values(batch,c.id,c.code,c.input_text,'completed',cmp->>'classification',coalesce(cmp->'labels','[]'::jsonb),pa,ca,pp,cp,cmp,'grammar-structural-shadow-v29','grammar-structural-shadow-v30');
  exception when others then
   ...
   values(batch,c.id,c.code,c.input_text,'error',null,'[]','{}','{}','{}','{}','{}','grammar-structural-shadow-v29','grammar-structural-shadow-v30',sqlerrm);
  end;
 end loop;
