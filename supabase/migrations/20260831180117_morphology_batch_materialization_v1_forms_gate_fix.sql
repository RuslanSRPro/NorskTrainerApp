...
as $$
declare tests jsonb:='[]'::jsonb; c int; rd jsonb; fd jsonb; x jsonb; passc int;
begin
 ...
 select count(*) into c from public.grammar_runtime_manifests where validated_by='morphology-batch-v1' and authoring_status='validated'; tests:=tests||jsonb_build_array(jsonb_build_object('code','manifests.six_validated','passed',c=6,'actual',c));
 select count(*) into c from public.grammar_rules gr join public.grammar_runtime_manifests m on m.id=gr.runtime_manifest_id where m.validated_by='morphology-batch-v1'; tests:=tests||jsonb_build_array(jsonb_build_object('code','rules.six_materialized','passed',c=6,'actual',c));
 select count(*) into c from public.grammar_rules gr join public.grammar_runtime_manifests m on m.id=gr.runtime_manifest_id where m.validated_by='morphology-batch-v1' and gr.is_active; tests:=tests||jsonb_build_array(jsonb_build_object('code','rules.six_globally_inactive','passed',c=0,'actual_active',c));
 select count(*) into c from public.grammar_rule_sources gs join public.grammar_rules gr on gr.id=gs.grammar_rule_id join public.grammar_runtime_manifests m on m.id=gr.runtime_manifest_id where m.validated_by='morphology-batch-v1' and gs.verification_status='source_verified'; tests:=tests||jsonb_build_array(jsonb_build_object('code','provenance.six_source_links','passed',c=6,'actual',c));
 select count(distinct gs.candidate_id) into c from public.grammar_rule_sources gs join public.grammar_rules gr on gr.id=gs.grammar_rule_id join public.grammar_runtime_manifests m on m.id=gr.runtime_manifest_id where m.validated_by='morphology-batch-v1'; tests:=tests||jsonb_build_array(jsonb_build_object('code','provenance.three_candidates','passed',c=3,'actual',c));
 select count(*) into c from public.grammar_rules gr join public.grammar_runtime_manifests m on m.id=gr.runtime_manifest_id where m.validated_by='morphology-batch-v1' and coalesce((public.assess_runtime_rule_execution_v5(gr.id)->>'ready_without_runtime_code_change')::boolean,false); tests:=tests||jsonb_build_array(jsonb_build_object('code','execution.six_ready','passed',c=6,'actual',c));
 ...
 tests:=tests||jsonb_build_array(jsonb_build_object('code','regression.forms_tenses_degrees_52_52','passed',(fd->>'passed')::int=52 and (fd->>'failed')::int=0,'actual',jsonb_build_object('total',fd->'total','passed',fd->'passed','failed',fd->'failed','batch_id',fd->'batch_id')));
 select count(*) into c from public.grammar_rules where code like 'nrg_rt_v1.%' and is_active; tests:=tests||jsonb_build_array(jsonb_build_object('code','architecture.active_nrg_zero','passed',c=0,'actual',c));
 ...
 return jsonb_build_object('version','morphology-batch-materialization-golden-v1','release_code',p_release_code,'tests',tests,'summary',jsonb_build_object('total',jsonb_array_length(tests),'passed',passc,'failed',jsonb_array_length(tests)-passc),'readiness',rd#>'{summary}','forms_tenses_degrees',jsonb_build_object('total',fd->'total','passed',fd->'passed','failed',fd->'failed','batch_id',fd->'batch_id'));
end;
$$;
