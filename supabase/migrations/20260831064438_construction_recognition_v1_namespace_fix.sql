...
declare
  ...
  t jsonb; t2 jsonb; m jsonb; m2 jsonb;
  i integer; n integer:=jsonb_array_length(v_tokens);
  pos1 text; pos2 text; v_candidate_status text; c jsonb;
  modal1 boolean; modal2 boolean; cop1 boolean; auxha1 boolean; auxbli1 boolean;
  forms2 jsonb; pred_types jsonb;
begin
  if not exists(select 1 from public.grammar_runtime_releases r where r.code=p_release_code and r.status in ('build','golden','shadow')) then
    ...
    if i<n then
      ...
      if cop1 and pos1='verb' then
        ...
        if jsonb_array_length(pred_types)>0 then
          v_candidate_status:=case when jsonb_array_length(pred_types)=1 then 'recognized' else 'hypothesis' end;
          c:=jsonb_build_object(
            'id',format('cr1:copular_predicative:%s:%s',i,i+1),
            'family','copular_predicative','status',v_candidate_status,'span_start',i,'span_end',i+1,
      ...
      if modal1 and modal2 and pos1='verb' and pos2='verb' then
        c:=jsonb_build_object(
